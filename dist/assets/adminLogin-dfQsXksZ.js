...
sessionStorage.getItem("tripgoAdmin") && location.replace("/admin.html");
const n = document.querySelector("#adminLogin");
n.innerHTML =
...
document.querySelector("#togglePassword").onclick = () => {
  ...
  ((e.type = e.type === "password" ? "text" : "password"),
    (document.querySelector("#togglePassword").textContent =
      e.type === "password" ? "Hiện" : "Ẩn"));
};
document.querySelector("#loginForm").onsubmit = async (e) => {
  e.preventDefault();
  const t = e.target.querySelector('button[type="submit"]'),
    s = document.querySelector("#error");
  ((t.disabled = !0), (t.textContent = "Đang xác thực..."));
  try {
    const a = Object.fromEntries(new FormData(e.target)),
      o = await (
        await fetch(
          `/api/users?email=${encodeURIComponent(a.email)}&password=${encodeURIComponent(a.password)}&role=admin`,
        )
      ).json();
    if (!o.length) throw new Error();
    ...
    ((s.textContent = "Email hoặc mật khẩu quản trị không chính xác."),
      s.classList.remove("hidden"),
      (t.disabled = !1),
      (t.textContent = "Đăng nhập hệ thống"));
  }
};
